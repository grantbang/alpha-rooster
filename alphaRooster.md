...
Version: 8.0 (Methodical Execution Build) Classification: Autonomous Programmatic Arbitrage Engine Target Infrastructure: Google Cloud Platform (GCP) Core Logic: Closed-Loop Yield Optimization
...
Call Feed API: Get categories where EPC > $2.00.

Call Google Ads Planner: Get AvgCPC for keywords in those categories.

Selection Rule: IF (EPC / AvgCPC) > 2.0 THEN Add to Target_List.
...
Tracker Validation: IF gclid is missing, generate UUID (Fail Open) & Log Warning.

AI Context: Call Vertex AI. Prompt: "Write 2 sentences defining '{q}' for a consumer." Timeout: 1.5s.

Feed Fetch: Call Feed API. CRITICAL: Pass gclid into subid4 parameter.

Render: Return Jinja2 HTML with {{ context }} and {{ feed_results }}.
